 ...
 * do puzzel sudoku e os insere numa 
 * matriz.
 */
 ...
 * existe_no_quadro_3x3 para verificar se o numero num pode
...
/**
 * Faz a busca por uma posicao vazia para inserio o numero num 
...
/**
 * Esta e a funcao que resolve o puzzel utilizando
...
int solucao_sudoku(int quadro[TAM][TAM], int modo_analise) {
    ...
    for (num = 1; num <= TAM; num++ ) {
        ...
        if (verifica_posicao(quadro, linha, coluna, num)) {
